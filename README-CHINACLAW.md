...
**跨平台 (Node.js):**
```bash
pnpm chinaclaw:oneclick
```

脚本会自动完成：安装依赖 → 构建项目 → 加载 `.env` → 启动网关 → 打开浏览器。

### 启动参数

| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--port` / `-Port` | 网关端口 | 18789 |
| `--no-browser` / `-NoBrowser` | 不自动打开浏览器 | 关闭 |
| `--rebuild` / `-Rebuild` | 强制重新构建 | 关闭 |
...
# Linux/macOS
./start.sh --port 8080 --no-browser

# 跨平台
pnpm chinaclaw:oneclick -- --port 8080 --no-browser
```
...
首次启动自动在 `~/.chinaclaw/openclaw.json` 创建默认配置。
